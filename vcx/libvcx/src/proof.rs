...
//        proof_compliance(&proof_req_msg.proof_request_data, &proof_msg)?;
    ...
    fn send_proof_request(&mut self, connection_handle: u32) -> Result<u32, ProofError> {
        ...
        let data = connection::generate_encrypted_payload(&self.prover_vk, &self.remote_vk, &proof_request.to_string(), "PROOF_REQUEST").map_err(|_| ProofError::ProofConnectionError())?;
...
mod tests {
    ...
    use messages::proofs::proof_request::tests::populated_proof_req_builder;
    ...
    fn test_proof_validation_with_predicate() {
        ...
        let mut proof_req_msg = populated_proof_req_builder().build().unwrap();
    ...
    fn test_proof_verification() {
        ...
        let mut proof_req_obj = populated_proof_req_builder().build().unwrap();
